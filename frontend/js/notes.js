// notes.js
export async function loadNotes(username, apiFetch){
  const pane = document.getElementById('notes');
  pane.innerHTML = '<h2>Notes</h2><p class="muted">Personal practice notes per day.</p>';
  const resp = await apiFetch(`/api/notes/${username}`);
  ...
  for(let d=1; d<=31; d++){
    ...
    ta.addEventListener('change', async (e) => {
      await apiFetch(`/api/notes/${username}`, { method: 'POST', body: JSON.stringify({ day: d, text: e.target.value }) });
    });
